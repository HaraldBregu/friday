...
export default function A2aPage(): React.JSX.Element {
	...
	return (
		<SettingsPageShell>
			<SettingsPageHeader title="A2A agents" description="Connect remote Agent2Agent-compatible agents and delegate work to them." action={<Button size="xs" onClick={() => setAdding(true)}><Plus className="size-3" />Add agent</Button>} />
			{notice && <SettingsNotice variant={notice.error ? 'destructive' : undefined}>{notice.text}</SettingsNotice>}
