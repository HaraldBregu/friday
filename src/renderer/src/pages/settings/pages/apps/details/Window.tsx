...
export default function WindowSettings({ appId }: { readonly appId: string }): React.JSX.Element {
	...
	useEffect(() => {
		...
		setError('');
		setStatus('');
		void window.apps.getSettings(appId).then((settings) => {
			if (!active) return;
			setSaved(settings);
			setDraft(settings);
		}).catch(() => {
	...
	return (
		<SettingsSection title={t('settings.apps.window.title')} description={t('settings.apps.window.nextOpen')}>
			{error && (
				<SettingsNotice variant="destructive">
					{t(`settings.apps.window.${error}`)}
					{error === 'loadError' && (
						<Button variant="outline" size="xs" className="ml-2" onClick={() => setAttempt(attempt + 1)}>
							{t('settings.apps.refresh')}
						</Button>
					)}
				</SettingsNotice>
			)}
			{loading ? <SettingsPanel><SettingsLoadingRows rows={3} /></SettingsPanel> : draft && error !== 'loadError' && (
					...
					<fieldset disabled={saving} className="grid min-w-0 gap-2">
						<SettingsPanel>
							<div className="grid grid-cols-1 gap-3 border-b border-border/60 p-3 sm:grid-cols-2">
								{DIMENSIONS.map((key) => (
									<SettingsField key={key} id={`${id}-${key}`} label={t(`settings.apps.window.${key}`)}>
										<Input id={`${id}-${key}`} type="number" min={1} max={32768} step={1} required
											className="h-8 text-xs" value={draft[key]} aria-invalid={!valid}
											aria-describedby={!valid ? `${id}-validation` : undefined}
											onChange={(event) => { setDraft({ ...draft, [key]: event.target.value }); setStatus(''); }} />
									</SettingsField>
								))}
							</div>
							{TOGGLES.map((key) => (
								<SettingsRow key={key} title={t(`settings.apps.window.${key}`)} actions={
									<Switch checked={draft[key]} aria-label={t(`settings.apps.window.${key}`)}
										onCheckedChange={(checked) => { setDraft({ ...draft, [key]: checked }); setStatus(''); }} />
								} />
							))}
						</SettingsPanel>
						{!valid && <p id={`${id}-validation`} role="alert" className="text-xs text-destructive">{t('settings.apps.window.invalid')}</p>}
						<div className="flex flex-wrap items-center justify-end gap-2">
							<p role="status" className="mr-auto text-xs text-muted-foreground">{status && t(`settings.apps.window.${status}`)}</p>
							<Button type="button" variant="outline" size="xs" onClick={() => void persist(true)}>{t('settings.apps.window.reset')}</Button>
							<Button type="submit" size="xs" disabled={!valid || !dirty}>{t(`settings.apps.window.${saving ? 'saving' : 'save'}`)}</Button>
						</div>
					</fieldset>
				</form>
			)}
