...
import { SupabaseProviderCloud } from './remote';
...
export class ProviderSyncService {
	...
	constructor(
		...
		private readonly cloud: ProviderCloudPort = new SupabaseProviderCloud(auth)
	) {}
	...
	initialize(): void {
		...
		this.unsubscribeAuth = this.auth.onStateChanged((state) => {
			if (state.status === 'signedIn' || state.status === 'recovery') this.requestRefresh();
		});
	...
	async setup(passphrase: string): Promise<ProviderVaultStatus> {
		...
		if (await this.cloud.getVault()) throw new Error('A provider sync vault already exists.');
		...
		try {
			...
			await this.cloud.createVault(identity.vaultId, envelope);
	...
	async unlock(passphrase: string): Promise<ProviderVaultStatus> {
		this.requireSignedIn();
		this.requirePassphrase(passphrase);
		const remote = await this.cloud.getVault();
	...
	async changePassphrase(passphrase: string): Promise<ProviderVaultStatus> {
		...
		try {
			const envelope = await wrapProviderDataKey(identity.key, passphrase, identity.vaultId);
			await this.cloud.updateVault(identity.vaultId, envelope);
			return this.status();
		} finally {
			identity.key.fill(0);
		}
	...
	private async performSync(): Promise<void> {
		...
		const localByKey = new Map(
			this.vault.records().map((record) => [this.recordKey(record), record])
		);
		for (const remote of await this.cloud.listCredentials(remoteVault.vaultId)) {
			const local = localByKey.get(this.recordKey(remote));
		...
		const canonical = await Promise.all(dirty.map((record) => this.cloud.syncCredential(record)));
		for (const record of canonical) this.vault.putRemote(record);
	...
	private isSignedIn(): boolean {
		const state = this.auth.getState();
		return state.status === 'signedIn' || state.status === 'recovery';
	}
