...
it('rejects unknown update IDs before discovery', async () => {
	await expect(
		saveA2aAgent({ id: 'missing', name: '', url: 'https://agent.example' })
	).rejects.toThrow('A2A agent not found');
...
it('does not persist cards with incompatible transports or required extensions', async () => {
	...
	await expect(saveA2aAgent({ name: '', url: 'https://agent.example' })).rejects.toThrow(
	...
	mockDiscover.mockResolvedValueOnce({
		...card,
		capabilities: { streaming: true, extensions: [{ uri: 'urn:required', required: true }] },
	});
	await expect(saveA2aAgent({ name: '', url: 'https://agent.example' })).rejects.toThrow(
